...
The :class:`~lightning.core.app.LightningApp` runs a tree of one or more components that interact to create end-to-end applications. There are two kinds of components: :class:`~lightning.core.flow.LightningFlow` and :class:`~lightning.core.work.LightningWork`. This modular design enables you to reuse components created by other users.
...
The :class:`~lightning.core.work.LightningWork` component is a building block optimized for long-running jobs or integrating third-party services. LightningWork can be used for training large models, downloading a dataset, or any long-lasting operation.
...
The :class:`~lightning.core.flow.LightningFlow` component coordinates long-running tasks :class:`~lightning.core.work.LightningWork` and runs its children :class:`~lightning.core.flow.LightningFlow` components.
...
When running an app in the cloud, the :class:`~lightning.core.work.LightningWork` run on different machines. Lightning communicates any :class:`~lightning.core.work.LightningWork` state changes to the **event loop** which re-executes the flow with the newly-collected works' state.
...
.. code-block:: console
    ...
    $ lightning run app docs/source-lit/code_samples/quickstart/app_01.py
...
By default, the :class:`~lightning.core.flow.LightningFlow` is executed infinitely by the **Lightning Infinite Loop** and the :class:`~lightning.core.work.LightningWork` does not run in **parallel**,
meaning the **Lightning Infinite Loop** (a.k.a the flow) waits until that long-running work is completed to continue.

Similar to `React.js Components and Props <https://reactjs.org/docs/components-and-props.html>`_, the :class:`~lightning.core.work.LightningWork`
...
.. code-block:: console
    ...
    $ lightning run app docs/source-lit/code_samples/quickstart/app_02.py
      INFO: Your app has started. View it in your browser: http://127.0.0.1:7501/view
      # After you have clicked `run` on the UI.
      0.0 0.0
      ...
...
your :class:`~lightning.core.work.LightningWork`.
...
.. code-block:: console
    ...
    $ lightning run app docs/source-lit/code_samples/quickstart/app/app_0.py
      INFO: Your app has started. View it in your browser: http://127.0.0.1:7501/view
      # After you have clicked `run` on the UI.
      0.0, 0.0
      ...
