...
To allow every notebook to choose hardware, it needs to be set up in it's own :class:`~lightning.core.work.LightningWork`, but you can't know the number of notebooks user will need in advance. In this case you'll need to add ``LightningWorks`` dynamically at run time.
...
**OPTION 2:** Use the built-in Lightning classes :class:`~lightning.structures.Dict` or :class:`~lightning.structures.List`
