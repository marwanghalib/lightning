...
To solve this problem, Lightning introduces the :class:`~lightning.storage.Path` object.
This ensures that your code can run both locally and in the cloud.

The :class:`~lightning.storage.Path` object keeps track of the work which creates
...
them into a :class:`~lightning.storage.Path` object and makes them an attribute of your LightningWork.
...
:class:`~lightning.core.work.LightningWork`
component is that you can run it on different hardware
by providing :class:`~lightning.utilities.packaging.cloud_compute.CloudCompute` to
the ``__init__`` method of your :class:`~lightning.core.work.LightningWork`.
